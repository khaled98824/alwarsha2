import 'dart:async';
import 'dart:io';
import 'package:alwarsha2/models/massrofatModel.dart';
...
class _MassrofatFState extends State<MassrofatF> {
  ...
  sumMassrofatF(){
    ...
    Timer(Duration(milliseconds: 400),(){
      for(int i =0 ; i < allList.length ; i++){
        setState(() {
          sumMassrofat = sumMassrofat.toDouble() +  allList[i].statues ;
        });
      }
    });
                            ...
                            itemBuilder: (BuildContext context, position) {
                              ...
                              return SizedBox(
                                ...
                                child: Card(
                                  child: ListTile(
                                    title: Text(
                                      "${allList[position].statues.toString()} : المبلغ ",
  ...
  void searchYom (String searchStatment){
    ...
    Query query = massrofReference
        .child('massrofat:$tabelNameSet').orderByChild('name').
    ...
    query.once().then((snapshot){
      ...
      snapshot.value.forEach((key ,value){
        ...
        searchMassrofat.add(MassrofatModel(key, name, time, amount, description));
